...
<?php
$sql="SELECT si.first,si.last,si.contact,s.username,s.password,s.roll,s.email FROM student s, student_info si where s.studentid = si.studentid and s.username='$_SESSION[login_user]' ";
...
<label><h4><b style="font-family:cursive;">Roll No. </b></h4> </label>
...
if (isset($_POST['submit1'])) {
    ...
    $admin_id_query = "SELECT studentid FROM student WHERE username = ?";
    ...
    if ($studentid) {
        ...
        $sql1 = "UPDATE student SET picture = ?, username = ?, password = ?,roll= ? ,email = ? WHERE studentid = ?";
        $stmt1 = mysqli_prepare($db, $sql1);
        mysqli_stmt_bind_param($stmt1, "sssssi", $picture, $username, $password, $roll , $email, $studentid);
        ...
        $sql2 = "UPDATE student_info SET first = ?, last = ?, contact = ? WHERE studentid = ?";
        $stmt2 = mysqli_prepare($db, $sql2);
        mysqli_stmt_bind_param($stmt2, "sssi", $first, $last, $contact, $studentid);
        if (!mysqli_stmt_execute($stmt2)) {
            // Error updating admin details
            $message = "Error updating student details: " . mysqli_error($db);
            $message_type = "error";
        }

...
<div class="backbox" style="background-color: white; padding: 20px; border-radius: 8px; width: 350px; position: relative; height: 170px;">
    <form method="post" action="" style="display: flex; justify-content: space-between; align-items: center; margin-left: 150px;">

        <!-- Username Field -->
        <div style="display: flex; flex-direction: column; align-items: flex-start;">
            <label for="username" style="font-family: cursive;">Username:</label>
            <input type="text" name="username" id="username" required="" style="width: 150px; margin-right: 20px;">
        </div>

        <!-- Book ID Field -->
        <div style="display: flex; flex-direction: column; align-items: flex-start;">
            <label for="bookid" style="font-family: cursive;">Book ID:</label>
            <input type="text" name="bookid" id="bookid" required="" style="width: 150px; margin-right: 20px;">
        </div>

        <!-- Submit Button -->
        <button type="submit" name="update_status" style="background-color: skyblue; color: white; padding: 10px 20px; border: none; border-radius: 4px; width: 150px;">
            Update
        </button>
    </form>
</div>
...
if (isset($_POST['update_status'])) {
    ...
    $query = "SELECT * FROM issue_book WHERE username = '$username' AND bookid = '$bookid'";
    $result = mysqli_query($db, $query);

    if (mysqli_num_rows($result) > 0) {
        // Update fine status to 'paid'
        $update_query = "UPDATE fine SET status = 'paid' WHERE username = '$username'";
        if (mysqli_query($db, $update_query)) {
            echo "<p style='color: green;margin-left:50px;'>Fine status updated to paid for username: $username</p>";
        } else {
            echo "<p style='color: red;'>Error updating the status.</p>";
        }
    } else {
...
$q=mysqli_query($db,"SELECT *  from fine where s.studentid=si.studentid   and si.first like '%$_POST[search]%'");

if(mysqli_num_rows($q)==0)
{
echo "sorry! no student found with this name. ";
}
else
{

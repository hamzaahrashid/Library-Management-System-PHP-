...
<head>
    ...
    <style>
        table {
            ...
            padding: 0 20px;
            transition: margin-left 0.5s ease; /* Add transition for smooth movement */
        }

        #sidebar:not(.collapsed) ~ table {
            margin-left: 180px; /* Shift table right when sidebar is expanded */
        }
        ...
        #sidebar:not(.collapsed) ~ .search {
            margin-left: 180px; /* Shift search form right when sidebar is expanded */
        }
        ...
        #main-content {
            margin-left: 70px; /* Default margin when sidebar is collapsed */
            padding: 20px;
            transition: margin-left 0.5s ease;
        }
        #sidebar:not(.collapsed) ~ #main-content {
            margin-left: 250px; /* Shift main content right when sidebar is expanded */
        }
        ...
        #sidebar:not(.collapsed) ~ h2 {
            margin-left: 330px; /* Shift heading right when sidebar is expanded */
        }

        #sidebar.collapsed ~ h2 {
            margin-left: 150px; /* Reset margin when sidebar is collapsed */
        }
    </style>
...
<body>
    ...
    if (isset($_POST['delete_book'])) {
        ...
        $delete_related_queries = [
            "DELETE FROM book_authors WHERE bookid = ?",
            "DELETE FROM book_department WHERE bookid = ?",
            "DELETE FROM issue_book WHERE bookid = ?"
        ];
        
        foreach ($delete_related_queries as $query) {
        ...
        $delete_book_query = "DELETE FROM books WHERE bookid = ?";
        $stmt = mysqli_prepare($db, $delete_book_query);
        mysqli_stmt_bind_param($stmt, "s", $bookid);
        
        if (mysqli_stmt_execute($stmt)) {
            echo "<script>alert('Book deleted successfully!');</script>";
        } else {
            echo "<script>alert('Error deleting book: " . mysqli_error($db) . "');</script>";
        }
        mysqli_stmt_close($stmt);
    }
    ?>
    ...
    <br><br>
    <h2 style="font-family:cursive;font-size: 25px;"><strong>LIST OF BOOKS</strong></h2>
    <div class="search">
        ...
        if (isset($_POST['submit']) && !empty($_POST['search'])) {
            ...
            if (mysqli_num_rows($q) == 0) {
                ...
                while ($row = mysqli_fetch_assoc($q)) {
                    ...
                    echo "<td>";
                    echo "<form method='post' onsubmit='return confirm(\"Are you sure you want to delete this book?\");'>";
                    echo "<input type='hidden' name='bookid' value='" . $row['bookid'] . "'>";
                    echo "<button type='submit' name='delete_book' style='background-color: red; color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer;'>Delete</button>";
                    echo "</form>";
                    echo "</td>";
                    echo "</tr>";
                }

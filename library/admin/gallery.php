    ...
    <div class="search">
        <form class="navbar-form" method="GET" action="" name="form1">
            <select name="filter_type">
                ...
                <option value="author" <?php echo (isset($_GET['filter_type']) && $_GET['filter_type'] === 'author') ? 'selected' : ''; ?>>Author</option>
            ...
            <input 
                ...
                style="height: 20px; border: 1px solid #ccc; border-radius: 4px; padding: 5px;">
            <button 
    ...
    <div id="main-content">
        <?php if (isset($bookDetails)): ?>
            ...
            <div class="book-details">
                ...
                <img src="images/<?php echo htmlspecialchars($bookDetails['pic'] ?? 'default.jpg'); ?>" alt="<?php echo htmlspecialchars($bookDetails['title']); ?>" style="width: 100%; margin-top: 20px;">
            </div>
        <?php else: ?>
            <?php
            // Search logic with advanced filtering
            $isSearchActive = isset($db) && isset($_GET['submit3']) && !empty($_GET['search']);
            if ($isSearchActive) {
                $resultsPerPage = 10;
                $page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
                $search = $_GET['search'];
                // Validate filter_type
                $allowedFilters = ['all', 'title', 'author', 'department'];
                $filter_type = isset($_GET['filter_type']) && in_array($_GET['filter_type'], $allowedFilters) ? $_GET['filter_type'] : 'all';
                ...
                $filterLabel = ucfirst($filter_type === 'all' ? 'All Fields' : $filter_type);
                echo "<div class='search-result-info'>Showing results for $filterLabel: \"" . htmlspecialchars($search) . "\"</div>";

                if ($result->num_rows > 0) {
                    echo '<div class="gallery-container">';
                    while ($book = $result->fetch_assoc()) {
                        $imagePath = 'images/' . htmlspecialchars($book['pic'] ?? 'default.jpg');
                        ?>
                        <div class="book-box">
